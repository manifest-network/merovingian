...
- Mainnet gets server-rendered descriptive content, self-referencing canonicals, descriptive titles/meta descriptions, truthful structured data, sitemap, OpenAPI, agent documents, and MCP registry publication. On 2026-09-24 the operator's account was verified as a Search Console owner, the sitemap was submitted, and Google indexed `/` and `/about` the same day. These support discoverability without promising rankings or agent traffic.
