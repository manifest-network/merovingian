...
The direct exec-form healthcheck is `/usr/local/bin/merovingian-healthcheck`, compiled from [tools/healthcheck.c](../tools/healthcheck.c) in a separate Alpine build stage. Only the small native executable enters the final image; the compiler and source do not. Its root-owned mode is `0555`. It starts no shell, Node VM or child process, reads decimal `PORT` (8080 when unset or empty), and connects only to IPv4 loopback `/healthz`. It uses no DNS, proxy configuration, TLS, redirects or response-body processing. An HTTP/1.0 or HTTP/1.1 2xx status line (at most 256 bytes) and complete headers (at most 8 KiB including the status line) succeed; connection, HTTP, protocol and configuration errors fail quietly.

One `alarm(4)` covers the complete probe, including connection, request writes and status-line reads; slowly arriving bytes cannot renew the budget. Its signal handler exits with status 1. Signal handling can be delayed by scheduling, so Docker retains its separate five-second timeout. The 30-second interval, 15-second startup period and three-failure threshold are unchanged. Image acceptance pins the complete native command and exercises success, errors, quiet output, hung connections and slowly arriving status lines. The test watchdog allows 15 seconds and has its own diagnostic; elapsed deadline checks allow scheduler tolerance separately.
...
The [0.4.5 candidate](RELEASE-0.4.5.md) replaces the per-probe Node startup, retains readiness behavior, and uses a direct native executable. The [sanitized evidence](evidence/healthcheck-2026-09-21.json) identifies the exact candidate and records validation and CPU counters. It supersedes the initial shell/wget candidate and its measurements.

`npm run check` passed all 197 tests, metadata consistency, typechecking and build; all ten focused runtime-check tests passed after the final diagnostic changes. The final-image check passed 19 health scenarios, independent custom-port application health, SQLite persistence, permissions and shutdown. The image scan passed the existing advisory policy with the previously reported low, unfixed `elliptic` advisory still visible and no exceptions.

Each CPU comparison used two disposable images whose filesystem layers and all configuration except the healthcheck command were verified identical. The baseline was built with the original `CMD ["node", "-e", ...]` healthcheck from commit `feacd4e`; the candidate uses `CMD ["/usr/local/bin/merovingian-healthcheck"]`. Neither command invokes a shell. Both retain the 30-second probe interval, 512 MiB memory, read-only root, no external network, dropped capabilities and no-new-privileges. After healthy startup and ten seconds of settling, host reads of the container cgroup counters bracketed a 65-second idle window. No measuring processes ran inside those cgroups. Each window contained two successful scheduled probes and no visits.

| CPU limit | Probe | Window CPU time | Added throttled periods | Added throttled time |
| --- | --- | ---: | ---: | ---: |
| 0.1 | Node baseline | 557.4 ms | 54 | 9,310.4 ms |
| 0.1 | Native | 271.5 ms | 23 | 5,741.8 ms |
| 0.5 | Node baseline | 297.6 ms | 3 | 279.9 ms |
| 0.5 | Native | 44.2 ms | 0 | 0.0 ms |

At the lease's stated 0.5-CPU quota, the local candidate used 85% less CPU over the window and its throttling counter stayed flat. The stricter 0.1-CPU run improved CPU time by 51%, but did **not** meet the issue's expectation of a flat throttling counter at that limit. These are short observations of the whole container, including application work and Docker execution overhead; they do not establish a two-millisecond per-process cost or predict provider alert clearance.
