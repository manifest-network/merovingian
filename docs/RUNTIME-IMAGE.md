...
`npm run check` passed all 202 tests, metadata consistency, typechecking and build. The suite includes regression checks for all cadence settings, startup failure classification, synchronous server diagnostics and the wrapper process-group watchdog. The final-image check passed 32 health scenarios, independent custom-port application health, SQLite persistence, permissions and shutdown. Curl and libcurl are Alpine `8.22.0-r0`; the scan inventories 23 OS packages, including the client dependencies. The image scan passed the existing advisory policy with the previously reported low, unfixed `elliptic` advisory still visible and no exceptions.
