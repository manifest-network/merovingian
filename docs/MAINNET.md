...
## Current release — 0.4.7 (2026-09-24)

Merovingian **0.4.7 is live at [merovingian.manifest.network](https://merovingian.manifest.network)**
on the original lease `01a0b0eb-a2d6-7831-85d6-820bfdb9cfcd`. Following explicit
authorization of each step, provider release **9** is ready with manifest hash
`2bea31fdfc2e270870fd9fc2cae73d3b003116745df13d7fe4bbdd5da214a81d` and the public
image built, published and attested by the [Release image workflow](IMAGE-RELEASE.md):

```text
ghcr.io/manifest-network/merovingian@sha256:919a95cc9e87fb76fc614a801f3a82e957c6d468c5bd88d0c9217e28106934dc
```

Public read-only smoke acceptance passed at **2026-09-24T13:13:56.540Z**: health
reports `0.4.7`. Persistent counts stayed at **13 cookies, 8 sauna sessions, and
10 teas** (31 total), matching the pre-update baseline, with the original start
date **2026-09-17T20:29:18.301Z**. Only the image changed; the runtime environment,
counter path, UID/GID, and empty proxy trust were preserved. The release moves the
runtime to Manifest SDK `0.23.0` and manifestjs `4.0.0` without changing
application behavior. MCP Registry publication through the approval-gated workflow
completed at **2026-09-24T13:16:15.382776Z**; exact-version and latest records
confirmed active `0.4.7` metadata matching the committed `server.json`.
...
[sanitized evidence](evidence/release-0.4.7.json). The dated records below describe
...
Release **0.4.3** replaced the image on the existing lease with one provider update POST, no new lease, and no chain transaction. Provider release **5** reached ready. The original launch receipts and image/hash below remain historical evidence; the current 0.4.7 image and manifest are recorded above, and the update procedure is documented below.
...
### Updates, recovery, and restore

Use the [existing-lease update workflow](#existing-lease-update-workflow), which sends the published SDK's `updateLease` request, to replace the manifest on the existing **ACTIVE** lease. The workflow keeps the active release's runtime configuration and changes the immutable image digest, plus any explicitly reviewed ingress setting. It keeps one journal per target digest, so it moves only to a digest it has not journaled. For a digest this workflow already applied, `prepare` only reprints the old journal and `run` refuses with `another_update_requires_manual_reconciliation`, so the workflow cannot roll back to an earlier release. An update does not create another lease or change its on-chain metadata hash. A provider error or timeout can follow an applied update: inspect status and releases before deciding whether to reapply that same intended update. The initial launch command deliberately refuses changed configuration and does not perform updates.

**Recover from a bad release by rolling forward.** Revert or fix the change on `main`, release it as a new version through the [Release image workflow](IMAGE-RELEASE.md), and apply that verified image through this same workflow with its own authorization. The version bump changes the packaged `package.json`, so the rebuilt image always has a new digest. Roll-forward keeps every release check and costs one image build and scan. A release that changes persisted data, such as the visit-counter database, must keep its revert release able to read that data.
...
This section records the historical September 18 release; 0.4.7 is current.
...
The update tool checks the exact existing active lease, domain, locked rate, current provider release, reviewed public environment, and pinned image. From `0.4.7`, the pinned image is the digest published by the [Release image workflow](IMAGE-RELEASE.md). It uses SDK provider authentication and the update endpoint; it creates no lease and sends no chain transaction. Inspect the current `0.4.7` update with:

```sh
node --import tsx scripts/mainnet-update.ts status \
  --image ghcr.io/manifest-network/merovingian@sha256:919a95cc9e87fb76fc614a801f3a82e957c6d468c5bd88d0c9217e28106934dc \
  --helper "$PWD/.local/mainnet/bin/keyring-signer" \
  --home "$HOME/.manifest" \
  --key-name merovingian
```
...
- Completed: the permanent MCP endpoint is published as `io.github.manifest-network/merovingian` version `0.4.7`, matching the live runtime/card identity and version. A supervised agent found the earlier `0.4.2` listing by name and completed one authorized free visit; releases `0.4.3` through `0.4.7` used read-only acceptance. Keep OpenAPI, `/visit.md`, and `/llms.txt` aligned with the running API.
- Search Console: the operator's account owns the `manifest.network` Domain property through a DNS TXT record at the apex, and `merovingian.manifest.network` is its child property. A TXT record cannot sit at the hostname, which is a CNAME. Keep the apex record: deleting it revokes this ownership. Owners of the Domain property can temporarily hide pages of any `manifest.network` host from Google Search (Removals, about six months). The sitemap was accepted with two pages, and Google indexed `/` and `/about` on 2026-09-24. The remaining optional checks, the robots.txt report and later the Page indexing and Performance reports, are listed in the [evidence](evidence/search-console-2026-09-24.json).
...
- Exercise roll-forward recovery from a failed release, health monitoring, low-credit alerts, and provider outage behavior.
